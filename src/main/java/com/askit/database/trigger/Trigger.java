...
public class Trigger {
	...
	public static void setPointsForRightAnswerInPrivateBet(final long selectedAnswerID, final long questionID) {
	...
	public static void setPointsForCreatePublicQuestion(final long userID) {
		executorService.execute(new Runnable() {
			@Override
			public void run() {
				Connection connection = null;
	...
	public static void setPointsForAnsweringAPublicQuestion(final long userID, final long questionID) {
		executorService.execute(new Runnable() {
			@Override
			public void run() {
				Connection connection = null;
	...
	public static void setPointsForAnsweringAPrivateQuestion(final long questionID, final long userID) {
		executorService.execute(new Runnable() {
			@Override
			public void run() {
				Connection connection = null;
			...
			public void run() {
				...
				try {
					...
					if (definitionOfEnd != DefinitionOfEnd.END_BY_TIME.getID()) {
						...
						// Count all Users whitch have allready answerd the
						// questionID
				...
				} catch (final SQLException | DatabaseLayerException exception) {
					SQLUtil.rollbackSilently(connection);
					ExceptionHandler.getInstance().handleError(exception);
				} finally {
					SQLUtil.closeSilentlySQL(connection, preparedStatement, null);
