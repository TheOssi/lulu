...
public class PutRequest extends Request {
	 ...
	 */
	public PutRequest(final String pathInfo, final Map<String, String[]> parameters, final PrintWriter out) {
	...
	public void handleRequest() throws MissingParametersException, WrongHashException, DuplicateHashException, DatabaseLayerException,
		...
		if (matcher.find()) {
			if (parameters.containsKey(URLConstants.PARAMETERS_USERID) && parameters.containsKey(URLConstants.PARAMETERS_USERNAME)) {
			...
			} else if (parameters.containsKey(URLConstants.PARAMETERS_USERID) && parameters.containsKey(URLConstants.PARAMETERS_LANGUAGE)) {
				userID = Long.parseLong(parameters.get(URLConstants.PARAMETERS_USERID)[0]);
				language = parameters.get(URLConstants.PARAMETERS_LANGUAGE)[0];
				queryManager.setLanguage(userID, language);
			} else if (parameters.containsKey(URLConstants.PARAMETERS_USERID) && parameters.containsKey(URLConstants.PARAMETERS_PASSWORDHASH)) {
				userID = Long.parseLong(parameters.get(URLConstants.PARAMETERS_USERID)[0]);
				passwordHash = parameters.get(URLConstants.PARAMETERS_PASSWORDHASH)[0];
				queryManager.setPasswordHash(userID, passwordHash);
			} else if (parameters.containsKey(URLConstants.PARAMETERS_USERID) && parameters.containsKey(URLConstants.PARAMETERS_PHONEHASH)) {
				userID = Long.parseLong(parameters.get(URLConstants.PARAMETERS_USERID)[0]);
				phoneNumberHash = parameters.get(URLConstants.PARAMETERS_PHONEHASH)[0];
				queryManager.setPhoneNumberHash(userID, phoneNumberHash);
			} else if (parameters.containsKey(URLConstants.PARAMETERS_USERID) && parameters.containsKey(URLConstants.PARAMETERS_PICTUREURL)) {
				userID = Long.parseLong(parameters.get(URLConstants.PARAMETERS_USERID)[0]);
		...
		if (matcher.find()) {
			if (parameters.containsKey(URLConstants.PARAMETERS_QUESTIONID) && parameters.containsKey(URLConstants.PARAMETERS_ANSWERID)
					&& parameters.containsKey(URLConstants.PARAMETERS_PUBLIC) && !parameters.containsKey(URLConstants.PARAMETERS_USERID)) {
				...
				if (!isPublic) {
					...
					// TODO only at bet (?)
					Trigger.setPointsForRightAnswerInPrivateBet(answerID, questionID);
					final Notification not = new Notification("", NotificationCodes.NOTIFICATION_ANSWER_SET.getCode(), "questionID",
							questionID.toString());
					NotificationSupporter.sendNotificationToAllMembersOfAGroup(not, groupID);
				}

			} else if (parameters.containsKey(URLConstants.PARAMETERS_QUESTIONID) && parameters.containsKey(URLConstants.PARAMETERS_ANSWERID)
					&& parameters.containsKey(URLConstants.PARAMETERS_PUBLIC) && parameters.containsKey(URLConstants.PARAMETERS_USERID)) {
				isPublic = Boolean.parseBoolean(parameters.get(URLConstants.PARAMETERS_PUBLIC)[0]);
				questionID = Long.parseLong(parameters.get(URLConstants.PARAMETERS_QUESTIONID)[0]);
				answerID = Long.parseLong(parameters.get(URLConstants.PARAMETERS_ANSWERID)[0]);
				...
				if (!isPublic) {
					...
					Trigger.setPointsForAnsweringAPrivateQuestion(questionID, userID);
					Trigger.afterAnsweringCheckForEndOfQuestionAndSetPoints(questionID);
				} else {
					queryManager.setChoosedAnswerOfPublicQuestion(userID, questionID, answerID);
					Trigger.setPointsForAnsweringAPublicQuestion(userID, questionID);
					Trigger.afterAnsweringCheckForEndOfQuestionAndSetPoints(questionID);
				}
			} else {
				throw new MissingParametersException();
		...
		{
			if (parameters.containsKey(URLConstants.PARAMETERS_GROUPID) && parameters.containsKey(URLConstants.PARAMETERS_USERID)) {
			...
			} else if (parameters.containsKey(URLConstants.PARAMETERS_GROUPID) && parameters.containsKey(URLConstants.PARAMETERS_GROUPNAME)) {
				groupID = Long.parseLong(parameters.get(URLConstants.PARAMETERS_GROUPID)[0]);
